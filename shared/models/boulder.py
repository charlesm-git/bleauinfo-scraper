...
class Boulder(Base):
    ...
    grade: Mapped["shared.models.grade.Grade"] = relationship(
        "Grade", back_populates="boulders", foreign_keys=[grade_id]
    )
    slash_grade: Mapped[Optional["shared.models.grade.Grade"]] = relationship(
        "Grade", foreign_keys=[slash_grade_id]
    )
    area: Mapped["shared.models.area.Area"] = relationship(
